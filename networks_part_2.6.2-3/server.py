...
def send_no_length(client_socket):
    reply = "You need to send the length first :("
    send_reply(client_socket, reply)


def get_send_msg(client_socket):
    length_client_msg = client_socket.recv(2).decode()

    if not length_client_msg.isnumeric():
        send_no_length(client_socket)
        return

    length_client_msg = int(length_client_msg)
